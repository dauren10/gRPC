...
type GreetServiceClient interface {
	...
	SayHelloByDirectionalStreaming(ctx context.Context, opts ...grpc.CallOption) (GreetService_SayHelloByDirectionalStreamingClient, error)
}
...
func (c *greetServiceClient) SayHelloByDirectionalStreaming(ctx context.Context, opts ...grpc.CallOption) (GreetService_SayHelloByDirectionalStreamingClient, error) {
	stream, err := c.cc.NewStream(ctx, &GreetService_ServiceDesc.Streams[2], "/greet_service.GreetService/SayHelloByDirectionalStreaming", opts...)
	if err != nil {
		return nil, err
	}
	x := &greetServiceSayHelloByDirectionalStreamingClient{stream}
	return x, nil
}

type GreetService_SayHelloByDirectionalStreamingClient interface {
	Send(*HelloRequest) error
	Recv() (*HelloResponse, error)
	grpc.ClientStream
}

type greetServiceSayHelloByDirectionalStreamingClient struct {
	grpc.ClientStream
}

func (x *greetServiceSayHelloByDirectionalStreamingClient) Send(m *HelloRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *greetServiceSayHelloByDirectionalStreamingClient) Recv() (*HelloResponse, error) {
	m := new(HelloResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type GreetServiceServer interface {
	...
	SayHelloByDirectionalStreaming(GreetService_SayHelloByDirectionalStreamingServer) error
...
func (UnimplementedGreetServiceServer) SayHelloByDirectionalStreaming(GreetService_SayHelloByDirectionalStreamingServer) error {
...
func _GreetService_SayHelloByDirectionalStreaming_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(GreetServiceServer).SayHelloByDirectionalStreaming(&greetServiceSayHelloByDirectionalStreamingServer{stream})
}

type GreetService_SayHelloByDirectionalStreamingServer interface {
	Send(*HelloResponse) error
	Recv() (*HelloRequest, error)
	grpc.ServerStream
}

type greetServiceSayHelloByDirectionalStreamingServer struct {
	grpc.ServerStream
}

func (x *greetServiceSayHelloByDirectionalStreamingServer) Send(m *HelloResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *greetServiceSayHelloByDirectionalStreamingServer) Recv() (*HelloRequest, error) {
	m := new(HelloRequest)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
